...
PLATFORM_SCHEMA = MEDIA_PLAYER_PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_MODES, default=CONF_MODES_DEFAULT): {cv.string: cv.string},
    }
)

SUPPORT_ONKYO_WO_VOLUME_WO_MODE = (
    MediaPlayerEntityFeature.TURN_ON
    | MediaPlayerEntityFeature.TURN_OFF
    | MediaPlayerEntityFeature.SELECT_SOURCE
    | MediaPlayerEntityFeature.PLAY_MEDIA
)
SUPPORT_ONKYO_WO_MODE = (
    SUPPORT_ONKYO_WO_VOLUME_WO_MODE
...
SUPPORT_ONKYO_WO_VOLUME = (
    SUPPORT_ONKYO_WO_VOLUME_WO_MODE | MediaPlayerEntityFeature.SELECT_SOUND_MODE
)
SUPPORT_ONKYO = SUPPORT_ONKYO_WO_MODE | MediaPlayerEntityFeature.SELECT_SOUND_MODE
...
def _mode_lib_cmds(zone: str) -> dict[ListeningMode, InputLibValue]:
    match zone:
        case "main":
    ...
    result: dict[ListeningMode, InputLibValue] = {}
    if cmds is not None:
        for k, v in cmds["values"].items():
            try:
                mode = ListeningMode(k)
            except ValueError:
                continue
            result[mode] = v["name"]
...
async def async_setup_platform(
    ...
    mode_mapping: dict[str, ListeningMode] = {}
    for zone in ZONES:
        mode_lib_cmds_zone = _mode_lib_cmds(zone)
        if mode_lib_cmds_zone is not None:
            for mode, mode_lib in mode_lib_cmds_zone.items():
                if isinstance(mode_lib, str):
                    mode_mapping.setdefault(mode_lib, mode)
                else:
                    for mode_lib_single in mode_lib:
                        mode_mapping.setdefault(mode_lib_single, mode)

    modes: dict[ListeningMode, str] = {}
    for mode_lib_single, mode_name in config[CONF_MODES].items():
        user_mode = mode_mapping.get(mode_lib_single.lower())
        if user_mode is not None:
            modes[user_mode] = mode_name

    config[CONF_MODES] = modes
...
class OnkyoMediaPlayer(MediaPlayerEntity):
    ...
    def __init__(
        ...
        *,
        ...
        self._mode_mapping = modes
        self._reverse_name_mapping = {value: key for key, value in sources.items()}
        self._reverse_mode_mapping = {value: key for key, value in modes.items()}
        self._lib_mapping = _input_lib_cmds(zone)
        self._mode_lib_mapping = _mode_lib_cmds(zone)
        self._reverse_lib_mapping = {
            value: key for key, value in self._lib_mapping.items()
        }
        self._reverse_mode_lib_mapping = {
            value: key for key, value in self._mode_lib_mapping.items()
        }

        self._attr_source_list = list(sources.values())
        self._attr_sound_mode_list = list(modes.values())
    ...
    def supported_features(self) -> MediaPlayerEntityFeature:
        ...
        if self._supports_volume:
            ...
            return SUPPORT_ONKYO_WO_MODE
        if self.sound_mode_list:
            return SUPPORT_ONKYO_WO_VOLUME
        return SUPPORT_ONKYO_WO_VOLUME_WO_MODE
    ...
    async def async_select_sound_mode(self, sound_mode: str) -> None:
        ...
        if self.sound_mode_list and sound_mode in self.sound_mode_list:
            mode_lib = self._mode_lib_mapping[self._reverse_mode_mapping[sound_mode]]
            if isinstance(mode_lib, str):
    ...
    def process_update(self, update: tuple[str, str, Any]) -> None:
        ...
        elif command in ["listening-mode"]:
    ...
    @callback
    def _parse_mode(self, mode_lib: InputLibValue) -> None:
        if mode_lib == "N/A":
            self._attr_sound_mode = None
            return

        mode = self._reverse_mode_lib_mapping[mode_lib]
        if mode in self._mode_mapping:
            self._attr_sound_mode = self._mode_mapping[mode]
            return

        mode_meaning = mode.value_meaning
        _LOGGER.error(
            'Listening mode "%s" not in mode list: %s', mode_meaning, self.entity_id
        )
        self._attr_sound_mode = mode_meaning
